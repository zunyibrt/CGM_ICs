...
//             P[i].Metallicity[0] = All.InitMetallicityinSolar*All.SolarAbundances[0];
    ...
    if(RestartFlag==0)
    {
        for(i = 0; i < N_gas; i++)
        {
            double Rc = 20., beta=0.5;
            int jk; double dp[3],r2d=0,r3d=0;
            for(jk=0;jk<3;jk++) {dp[jk] = P[i].Pos[jk] - 0.5*All.BoxSize;}
            for(jk=0;jk<3;jk++) {r3d += dp[jk]*dp[jk];}
            for(jk=0;jk<2;jk++) {r2d += dp[jk]*dp[jk];}
            r2d=sqrt(r2d); r3d=sqrt(r3d);
#ifdef METALS
            P[i].Metallicity[0] = 0.0129;
            if (r3d>10) P[i].Metallicity[0] = 0.0129 * 0.300;
            for(j=0;j<NUM_METAL_SPECIES;j++) P[i].Metallicity[j]=All.SolarAbundances[j]*P[i].Metallicity[0]/All.SolarAbundances[0];
            if(NUM_METAL_SPECIES>=10) P[i].Metallicity[1]=0.25+(All.SolarAbundances[1]-0.25)*P[i].Metallicity[0]/All.SolarAbundances[0];
#endif
#ifdef MAGNETIC
            double B0 = All.BiniZ * a2_fac*gauss2gizmo / pow((1+pow(r3d/Rc,2)),0.75*beta);
            //double B0 = All.BiniZ * a2_fac*gauss2gizmo / (1. + pow(r3d/scale_length,2));
            SphP[i].BPred[2]=0; SphP[i].BPred[1]=B0*dp[0]/r2d; SphP[i].BPred[0]=-B0*dp[1]/r2d; // azimuthal field: R,z dependence are free
            //for(jk=0;jk<3;jk++) {SphP[i].BPred[jk] = dp[jk]*scale_length*scale_length/(r3d*r3d*r3d) * All.BiniZ*a2_fac*gauss2gizmo;} // radial field: only solution is B~1/r^2
            //SphP[i].BPred[0]=SphP[i].BPred[1]=0; SphP[i].BPred[2]=All.BiniZ * a2_fac*gauss2gizmo; // z-field: only solution is constant Bz
#ifdef COSMIC_RAYS
            SphP[i].CosmicRayEnergyPred = SphP[i].CosmicRayEnergy = 1.0 * (3./2.)*(SphP[i].BPred[0]*SphP[i].BPred[0]+SphP[i].BPred[1]*SphP[i].BPred[1]+SphP[i].BPred[2]*SphP[i].BPred[2]) * P[i].Mass/SphP[i].Density;
#endif
#endif
        }
    }

// Xiangcheng: add the following blocks for output cooling
#if defined(OUTPUTCOOLRATE) && defined(COOLING)
    ...
    SphP[i].MyHeatT = 0.0;
#endif // if defined(OUTPUTCOOLRATE) && defined(COOLING)
...
//        All.MinMassForParticleMerger = 0.49 * 5.0e-05; 
...
//        All.MaxMassForParticleSplit  = 3.01 * 5.0e-05; 
