...
void GravAccel_StaticInfiniteIsothermalSphere(void);
...
void add_analytic_gravitational_forces()
{
    ...
    GravAccel_StaticInfiniteIsothermalSphere(); 
...
/* static Isothermal Sphere Profile (parameters specified in the routine below) */
void GravAccel_StaticInfiniteIsothermalSphere()
{
    double vc=200;
    double r2, dp[3];
    int i, k;
...
//        for(k = 0; k < 3; k++) {dp[k] = -P[i].min_xyz_to_bh[k];}
        r2 = dp[0]*dp[0] + dp[1]*dp[1] + dp[2]*dp[2];
        if(r2 > 0) {for(k=0;k<3;k++) {P[i].GravAccel[k] += -vc*vc * dp[k] / (r2 + ISO_Eps*ISO_Eps);}}
    }
}
