...
class ICs:
    ...
    outdir_template = outdir_data + 'vc%d_Rs%d_Mdot%d_Rcirc%d_res%s'
    ...
    def create_ICs_hdf5_file(self,fn_out):                
        ...
        try:        
            ...
            for iPartType in range(6):
                ...
                for k in ['pos', 'vel', 'ids', 'mass', 'energies']:
                    if iPartType==0:                
                        if k=="pos":
                            data   = gas_coords
                        if k=="energies":
                            data   = gas_internalEnergies.value
                        if k=="mass":                    
                            Zs = np.ones((len(gas_masses),11))*self.Z_CGM * Zsun
                            data   = gas_masses.value/MASS_UNITS
                            grp.create_dataset('Metallicity', Zs.shape, dtype=Zs.dtype)
                            grp['Metallicity'][:] = Zs                      
                        if k=='vel':
                            data   = gas_vels
                        if k=='ids':
                            data   = np.arange(len(gas_masses))                        
                    else:
                        data = np.array([])
                    if k=='mass': 
                        if data.shape[0]!=0:
            ...
            header = {}
            header['Npart'] = np.array([len(gas_masses),0,0,0,0,0])
            header['Mpart'] = np.array([0,0,0,0,0,0])
            header['Time'] = 0.0
