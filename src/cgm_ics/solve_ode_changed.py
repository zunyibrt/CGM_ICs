...
def _calculate_sonic_point_conditions_from_state(
    T_sonic: un.Quantity,
    rho_sonic: un.Quantity,
    R_sonic: un.Quantity,
    Mdot: un.Quantity,
    potential: Potential,
    cooling: Cooling,
    ...
    """
    Calculate sonic point conditions from known state variables.
    
    This is used when a new sonic point is encountered during integration,
    where we already know T, rho, and R at the sonic point.
    
    Args:
        T_sonic: Temperature at the sonic point.
        rho_sonic: Density at the sonic point.
        R_sonic: Radius of the sonic point.
        Mdot: Mass flow rate.
        potential: Object defining the gravitational potential.
        cooling: Object defining the cooling function.
    ...
    Returns:
        Dictionary containing sonic point conditions including dlnTdlnR, 
        dlnrhodlnR, and dlnMdlnR.
    """
    # Calculate x = vc^2 / (2 cs^2) from known state
    cs2 = (GC.GAMMA * cons.k_B * T_sonic / (GC.MU * cons.m_p)).to('km**2/s**2')
    vc2 = potential.vc(R_sonic)**2
    x = (vc2 / (2 * cs2)).to('').value
    
    # Calculate dlnT/dlnR at the sonic point using L'Hôpital's rule
    dlnTdlnR1, dlnTdlnR2 = _calc_dlnTdlnR_at_sonic_point(
        R_sonic, x, rho_sonic, T_sonic, cooling, potential
    )
...
def _integrate_flow_sonic(
    ...
    Args:
        ...
        verbose: If True, print integration progress information.
        max_sonic_restarts: Maximum number of times to restart from new sonic points.
    ...
    for direction in integration_directions:
        ...
        if direction == 1: # Integrate outwards
            # Track current state for potential sonic point restarts
            current_T = T_sonic_point
            current_rho = rho_sonic_point
            current_R_sonic = R_sonic
            current_dlnTdlnR = dlnTdlnR
            current_dlnrhodlnR = dlnrhodlnR
            current_dlnMdlnR = dlnMdlnR
            
            accumulated_results = []  # Store results from each segment
            sonic_restart_count = 0
            
            while sonic_restart_count < max_sonic_restarts:
                # Calculate starting conditions offset from sonic point
                T0 = current_T * (1.0 + epsilon * current_dlnTdlnR)
                rho0 = current_rho * (1.0 + epsilon * current_dlnrhodlnR)
                R0 = current_R_sonic * (1.0 + epsilon)
                isSupersonic = current_dlnMdlnR >= 0
                
                res_segment = IntegrateFlowEquations(
                ...
                accumulated_results.append(res_segment)

                printv(f'Stop reason: {res_segment.stopReason.value} (Maximum r = {int(res_segment.Rs[-1].to("kpc").value):d} kpc)', verbose)

                if res_segment.stopReason == StopReason.SONIC_POINT:
                    # Extract state at the new sonic point
                    new_R_sonic = res_segment.Rs[-1]
                    new_T_sonic = res_segment.Ts[-1]
                    new_rho_sonic = res_segment.rhos[-1]
                    
                    printv(f'  -> Restarting from new sonic point at R = {new_R_sonic.to("kpc").value:.2f} kpc', verbose)
                    
                    try:
                        # Calculate new sonic point conditions
                        new_conditions = _calculate_sonic_point_conditions_from_state(
                            new_T_sonic, new_rho_sonic, new_R_sonic, 
                            Mdot, potential, cooling, current_dlnMdlnR
                        )
                        
                        # Update state for next iteration
                        current_T = new_conditions['T_sp']
                        current_rho = new_conditions['rho_sp']
                        current_R_sonic = new_R_sonic
                        current_dlnTdlnR = new_conditions['dlnTdlnR']
                        current_dlnrhodlnR = new_conditions['dlnrhodlnR']
                        current_dlnMdlnR = new_conditions['dlnMdlnR']
                        
                        sonic_restart_count += 1
                        
                    except NoTranssonicSolutionError:
                        printv(f'  -> No transsonic solution at new sonic point, stopping', verbose)
                        break
                else:
                    # Not a sonic point stop, exit the loop
                    break
            
            # Combine all accumulated results into a single solution
            if len(accumulated_results) > 1:
                res = _combine_outward_results(accumulated_results, cooling, potential, Mdot)
            else:
                res = accumulated_results[0]
                
            if res.stopReason in (StopReason.TEMPERATURE_FLOOR, StopReason.UNBOUND):
                return res
...
def _combine_outward_results(
    results: List[CGMSolution],
    cooling: Cooling,
    potential: Potential,
    Mdot: un.Quantity
) -> CGMSolution:
    """
    Combine multiple outward integration segments into a single CGMSolution.
    
    Args:
        results: List of CGMSolution objects from successive integration segments.
        cooling: Cooling function object.
        potential: Gravitational potential object.
        Mdot: Mass flow rate.
        
    Returns:
        Combined CGMSolution object.
    """
    # Concatenate the integration results
    # Note: We skip the first point of each subsequent segment to avoid duplication
    # at the sonic point boundaries
    combined_t = results[0].result.t.copy()
    combined_y = results[0].result.y.copy()
    
    for res in results[1:]:
        # Skip the first point to avoid duplicate at boundary
        combined_t = np.concatenate([combined_t, res.result.t[1:]])
        combined_y = np.concatenate([combined_y, res.result.y[:, 1:]], axis=1)
    
    # Create a simple object to hold the combined result
    class CombinedResult:
        def __init__(self, t, y):
            self.t = t
            self.y = y
    
    combined_result = CombinedResult(combined_t, combined_y)
    
    # Use the stop reason from the final segment
    final_stop_reason = results[-1].stopReason
    
    return CGMSolution(cooling, potential, combined_result, Mdot, final_stop_reason, direction=1)
